...
void main()
{
  ...
  /**/
  //WILF: Change each 0.5 to 1.0 and use a simple average instead.
  /*
  pos[0] = texture2DRect(hiResPosTex, xy2 + vec2(-1.0, 1.0));
  pos[1] = texture2DRect(hiResPosTex, xy2 + vec2(1.0, 1.0));
  pos[2] = texture2DRect(hiResPosTex, xy2 + vec2(1.0, -1.0));
  pos[3] = texture2DRect(hiResPosTex, xy2 + vec2(-1.0, -1.0));

  norm[0] = texture2DRect(hiResNormTex, xy2 + vec2(-1.0, 1.0));
  norm[1] = texture2DRect(hiResNormTex, xy2 + vec2(1.0, 1.0));
  norm[2] = texture2DRect(hiResNormTex, xy2 + vec2(1.0, -1.0));
  norm[3] = texture2DRect(hiResNormTex, xy2 + vec2(-1.0, -1.0));
  
  Pos = (pos[0]+pos[1]+pos[2]+pos[3]) * 0.25;
  Norm = (norm[0]+norm[1]+norm[2]+norm[3]) * 0.25;
  return;
  */
